...
# Lấy mã lỗi từ Google Sheets và cache trong RAM
@lru_cache(maxsize=1)
def get_error_codes_from_sheets():
    try:
        ...
        logging.error(f"Lỗi khi lấy dữ liệu từ Google Sheets: {e}")
        return {}
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.info(f"Người dùng gửi: {user_input}")
...
def webhook():
    ...
    update = Update._from_data(data, application.bot)  # nếu dùng v20+
...
async def set_webhook():
    ...
    logging.info(f"✅ Đã thiết lập webhook: {webhook_url}")
